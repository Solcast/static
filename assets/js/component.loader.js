...
(() => {
  ...
  onReady(async () => {
    ...
    // --- Injectors ---
    const forceLoadScript = (src, opts) => new Promise((resolve, reject) => {
    ...
    const loadModule = (fileName, label) => {
      ...
      if (!loadModule.cache.has(fileName)) {
        ...
        loadModule.cache.set(fileName, import(url).then((m) => {
          m.default?.();
          return m;
        }).catch((e) => { throw e; }));
      }
      ...
      try {
        ...
        if (others.length) {
          await Promise.allSettled(
            others.map((src) =>
              forceLoadScript(src, RECAPTCHA_REGEX.test(src) ? { async: true, defer: true } : { async: true })
            )
          );
        }
      } catch (e) {
    ...
    if (has('#slider_logoCarousel')) {
      tasks.push(
        Promise.all([
          ...
          loadExternalScript('https://cdn.jsdelivr.net/npm/@splidejs/splide-extension-auto-scroll/dist/js/splide-extension-auto-scroll.min.js', 'splide-auto'),

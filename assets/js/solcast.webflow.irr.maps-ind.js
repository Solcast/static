...
getJSON(
  ...
  (err, data) => {
    ...
    // console.log('Something went wrong: ' + err);
  	...
  	// console.log(data);
    // console.log('Your query count: ' + JSON.stringify(data.files[0]));
    // console.log('Your query count: ' + data.files[0].video_url);
    ...
    + '    </div>';
      ...
      if (myParam != '') {
        document.querySelectorAll('#gallery-list-section .item .inner_block').forEach((load_summary) => {
          var get_item_id = load_summary.getAttribute('data-item-id');
          if (get_item_id == myParam) {
            // var get_item_id = load_summary.getAttribute('data-item-id'); unnecessary line
